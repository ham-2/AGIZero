/*
  ...
  First Implemented from Stockfish 13:
...
string Position::fen() const {
  ...
  ss << (ep_square() == SQ_NONE ? " - " : " " + AGI::square(ep_square()) + " ")
...
Key Position::real_key_after(Move m) const {
    ...
    {
        Square capsq = to;
        if (type_of(m) == EN_PASSANT) { capsq -= pawn_push(us); }
        k ^= Zobrist::psq[captured][capsq];
        rule50 = 0;
    }
